...
class NewOrderController extends Controller
{
    ...
    public function saveRequest(Request $request, $id)
    {
        ...
        if ($order != '0') {
            $msg = trans('theme.order_success') . " - " . trans('theme.order_no') . ":". $order[0];
...
//                    dd($discount_ch);
                ...
                $dis = 0;
                $discount_ = $item->withDiscount;
                if ($discount_) {
                    $dis = $item->discount_percent / 100 * $price_original *$quantity;
                    $price_new = (1 - ($item->discount_percent / 100)) * $price_original;
                    /* if ($item->vat_state == 2) {
                         $price_new = (1 - ($item->discount_percent / 100)) * $price;
                     }*/
...
//dd($item_total_price);
            ...
            $all_fee = $request->fee;
            if($request->has('fast_charge') && !in_array($request->fast_charge, ['',null]) && $request->fast_charge == '1' && $settings){

                if($settings->fee_type == 0){
                    $all_fee = $settings->fee / 100 * $net;
                }
            }
            ...
            $coupon_discount = 0;
            if ($request->has('coupon_id') && !in_array($request->coupon_id, ['', null, '0'])) {
                $coupon = Coupon::where('id', $request->coupon_id)->first();
                if ($coupon && $coupon->check()) {
                    if ($coupon->discount_type == 0) {
                        $coupon_discount = $coupon->discount_value * ($net + $all_fee) / 100;
                    } else {
            ...
            $requestc->coupon_id = $request->coupon_id ?? 0;
            ...
            $requestc->address = $request->address;

            $requestc->created_at = date('Y-m-d H:i:s');
            $requestc->updated_at = date('Y-m-d H:i:s');
...
//                if ($pay_type == '1') {
                ...
                if ($cardCheck != '0') {
                    $cards = $this->getCards($it, $data['quantity']);
                    //dd( $data['quantity'], count($cards), $cards);
...
//            return $data;
    ...
    protected function cardCheck($item)
    {
    ...
    public function checkCoupon($shop_id, Request $request)
    {
        ...
        if ($coupon) {
            if ($coupon->check()) {
                ...
                return response()->json([
                    'status' => true,
                    'message' => trans('theme.coupon_success'),
                    'coupon_id' => $coupon->id,
